...
function build_exaxmples() {
    ...
    go run examples/ceo1.go && \
    go run examples/solvelinear.go && \
    go run examples/logistic.go 
    ...
    go run examples/feasrepairex1.go examples/feasrepair.lp 
}


go build && \
    go run examples/parallel.go && \
    build_exaxmples
