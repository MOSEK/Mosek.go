...
const ( // enum rescode
    ...
    RES_ERR_SERVER_PROBLEM_SIZE int32 = 8008
...
func (task *Task) PutOptserverHost(host_ string) () {
  if task.r == RES_OK {
    _c_host_ := C.CString(host_)
    defer C.free(unsafe.Pointer(_c_host_))
    task.r = int32(C.MSK_putoptserverhost(task.ptr(),C.MSKstring_t(_c_host_)))
    if task.r != 0 { return }
  }
  return
}
